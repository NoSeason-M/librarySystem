...
let readerChart: echarts.ECharts | null = null
...
function initBorrowChart() {
  ...
  if (borrowChartData.value.length === 0) return
  borrowChart.setOption({
    ...
    xAxis: {
      type: 'category',
      data: borrowChartData.value.map(d => d.date),
...
function initReaderChart() {
  ...
  readerChart.setOption({
    ...
    series: [{
      ...
      label: { show: false },
      labelLine: { show: false },
      data: readerTypeDist.value.map((d, i) => ({
        name: d.name, value: d.count,
        itemStyle: { color: CATEGORY_COLORS[i % CATEGORY_COLORS.length] },
      })),
    }],
...
function initAllCharts() {
  nextTick(() => { initBorrowChart(); initCategoryChart(); initReaderChart() })
}
  ...
  <div class="admin-stats">
    <aside class="sidebar">
      ...
      <nav class="sidebar__nav">
        <div v-for="item in adminNav" :key="item.label" class="sidebar__item" :class="{ 'sidebar__item--active': item.label === 'Statistics' }" @click="navigateTo(item.label)"><span class="sidebar__item-icon">{{ item.icon }}</span><span>{{ item.label }}</span></div>
      </nav>
    ...
    <main class="main">
      <header class="header">
        <h1 class="header__title">Statistics</h1>
        <div class="header__tabs">
          <button :class="['tab-btn', { 'tab-btn--active': granularity === 'month' }]" @click="switchGranularity('month')">Monthly</button>
          <button :class="['tab-btn', { 'tab-btn--active': granularity === 'week' }]" @click="switchGranularity('week')">Weekly</button>
          <button :class="['tab-btn', { 'tab-btn--active': granularity === 'day' }]" @click="switchGranularity('day')">Daily</button>
        </div>
      </header>
      ...
      <template v-if="!loading">
        <div class="row">
          <div class="card card--lg">
            <div class="card__header"><h2 class="card__title">📊 Borrow Statistics</h2></div>
            <div class="summary-row">
              <div class="summary-item"><span class="summary-label">Total Borrow</span><span class="summary-value">{{ borrowSummary.totalBorrow }}</span></div>
              <div class="summary-item"><span class="summary-label">Total Return</span><span class="summary-value">{{ borrowSummary.totalReturn }}</span></div>
              <div class="summary-item"><span class="summary-label">Avg Daily</span><span class="summary-value">{{ borrowSummary.avgDailyBorrow }}</span></div>
              <div class="summary-item"><span class="summary-label">Peak Day</span><span class="summary-value" style="color:var(--accent,#4A9FD8)">{{ borrowSummary.peakDay }}</span></div>
            </div>
            <div ref="borrowChartRef" class="chart-borrow"></div>
          </div>

          <div class="card card--sm">
            <div class="card__header"><h2 class="card__title">🔥 Hot Books</h2></div>
            <div class="hot-list">
              <div v-for="(book, i) in hotBooks" :key="book.id" class="hot-item">
                <div :class="['hot-rank', { 'hot-rank--top': i < 3 }]">{{ book.rank }}</div>
                <div class="hot-info"><span class="hot-title">{{ book.title }}</span><span class="hot-author">{{ book.author }}</span></div>
                <span class="hot-count">{{ book.borrowCount }}</span>
              </div>
              <div v-if="hotBooks.length === 0" class="empty-msg">No data yet</div>
            </div>
          </div>
        </div>
        ...
        <div class="row">
          <div class="card card--lg">
            <div class="card__header"><h2 class="card__title">📚 Collection Stats</h2></div>
            <div class="summary-row">
              <div class="summary-item"><span class="summary-label">Total Books</span><span class="summary-value">{{ collectionStats.totalBooks }}</span></div>
              <div class="summary-item"><span class="summary-label">Total Copies</span><span class="summary-value">{{ collectionStats.totalCopies }}</span></div>
            </div>
            <div class="chart-row">
              <div class="donut-wrapper">
                <div ref="categoryChartRef" class="chart-donut"></div>
              </div>
              <div class="chart-legend-col">
                <div v-for="(d, i) in categoryData" :key="d.name" class="legend-item">
          ...
          <div class="card card--sm">
            <div class="card__header"><h2 class="card__title">👥 Reader Stats</h2></div>
            <div class="reader-metrics">
              <div class="reader-metric"><span class="metric-label">Total Registered</span><span class="metric-value">{{ readerStats.totalReaders }}</span></div>
              <div class="reader-metric"><span class="metric-label">Active (30 days)</span><span class="metric-value" style="color:var(--success,#34D399)">{{ readerStats.activeReaders }}</span></div>
              <div class="reader-metric"><span class="metric-label">Active Rate</span><span class="metric-value" style="color:var(--accent,#4A9FD8)">{{ readerStats.activeRate }}%</span></div>
            </div>
            <div class="chart-row chart-row--compact">
              <div class="donut-wrapper donut-wrapper--sm">
                <div ref="readerChartRef" class="chart-donut reader-chart"></div>
              </div>
              <div class="chart-legend-col">
                <div v-for="(d, i) in readerTypeDist" :key="d.name" class="legend-item">
                  <span class="legend-dot" :style="{ background: CATEGORY_COLORS[i % CATEGORY_COLORS.length] }"></span>
                  <span class="legend-label">{{ d.name }}</span>
                  <span class="legend-value">{{ d.percentage }}%</span>
                </div>
...
<style scoped>
.admin-stats { display: flex; min-height: 100vh; background: var(--bg-secondary, #F7F8FA); }
