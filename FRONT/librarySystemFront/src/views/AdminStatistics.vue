...
function initBorrowChart() {
  ...
  borrowChart.setOption({
    ...
    legend: { data: ['借出', '归还'], bottom: 0, textStyle: { fontSize: 11, color: '#888' } },
    ...
    series: [
      { name: '借出', type: 'bar', data: borrowChartData.value.map(d => d.borrowCount), itemStyle: { color: '#4A9FD8', borderRadius: [4, 4, 0, 0] }, barWidth: 10 },
      { name: '归还', type: 'bar', data: borrowChartData.value.map(d => d.returnCount), itemStyle: { color: '#34D399', borderRadius: [4, 4, 0, 0] }, barWidth: 10 },
    ],
  })
}
...
<template>
  <div class="admin-stats">
    ...
    <main class="main">
      <header class="header"><h1 class="header__title">统计分析</h1><div class="header__tabs"><button :class="['tab-btn', { 'tab-btn--active': granularity === 'month' }]" @click="switchGranularity('month')">按月</button><button :class="['tab-btn', { 'tab-btn--active': granularity === 'week' }]" @click="switchGranularity('week')">按周</button><button :class="['tab-btn', { 'tab-btn--active': granularity === 'day' }]" @click="switchGranularity('day')">按天</button></div></header>
      <div v-if="loading" class="loading-msg">加载统计数据中...</div>
      <template v-if="!loading">
        <div class="row">
          <div class="card card--lg"><div class="card__header"><h2 class="card__title">📊 借阅统计</h2></div><div class="summary-row"><div class="summary-item"><span class="summary-label">总借出</span><span class="summary-value">{{ borrowSummary.totalBorrow }}</span></div><div class="summary-item"><span class="summary-label">总归还</span><span class="summary-value">{{ borrowSummary.totalReturn }}</span></div><div class="summary-item"><span class="summary-label">日均</span><span class="summary-value">{{ borrowSummary.avgDailyBorrow }}</span></div><div class="summary-item"><span class="summary-label">峰值日</span><span class="summary-value" style="color:var(--accent,#4A9FD8)">{{ borrowSummary.peakDay }}</span></div></div><div ref="borrowChartRef" class="chart-borrow"></div></div>
          <div class="card card--sm"><div class="card__header"><h2 class="card__title">🔥 热门排行</h2></div><div class="hot-list"><div v-for="(book, i) in hotBooks" :key="book.id" class="hot-item"><div :class="['hot-rank', { 'hot-rank--top': i < 3 }]">{{ book.rank || (i + 1) }}</div><div class="hot-info"><span class="hot-title">{{ book.title }}</span><span class="hot-author">{{ book.author }}</span></div><span class="hot-count">{{ book.borrowCount }}</span></div><div v-if="hotBooks.length === 0" class="empty-msg">暂无数据</div></div></div>
        </div>
        <div class="row">
          <div class="card card--lg"><div class="card__header"><h2 class="card__title">📚 馆藏统计</h2></div><div class="summary-row"><div class="summary-item"><span class="summary-label">图书种类</span><span class="summary-value">{{ collectionStats.totalBooks }}</span></div><div class="summary-item"><span class="summary-label">总副本数</span><span class="summary-value">{{ collectionStats.totalCopies }}</span></div></div><div class="chart-row"><div class="donut-wrapper"><div ref="categoryChartRef" class="chart-donut"></div></div><div class="chart-legend-col"><div v-for="(d, i) in categoryData" :key="d.name" class="legend-item"><span class="legend-dot" :style="{ background: CATEGORY_COLORS[i % CATEGORY_COLORS.length] }"></span><span class="legend-label">{{ d.name }}</span><span class="legend-value">{{ d.percentage }}%</span></div></div></div></div>
          <div class="card card--sm"><div class="card__header"><h2 class="card__title">👥 读者统计</h2></div><div class="reader-metrics"><div class="reader-metric"><span class="metric-label">总注册</span><span class="metric-value">{{ readerStats.totalReaders }}</span></div><div class="reader-metric"><span class="metric-label">活跃 (30天)</span><span class="metric-value" style="color:var(--success,#34D399)">{{ readerStats.activeReaders }}</span></div><div class="reader-metric"><span class="metric-label">活跃率</span><span class="metric-value" style="color:var(--accent,#4A9FD8)">{{ readerStats.activeRate }}%</span></div></div><div class="chart-row chart-row--compact"><div class="donut-wrapper donut-wrapper--sm"><div ref="readerChartRef" class="chart-donut reader-chart"></div></div><div class="chart-legend-col"><div v-for="(d, i) in readerTypeDist" :key="d.name" class="legend-item"><span class="legend-dot" :style="{ background: CATEGORY_COLORS[i % CATEGORY_COLORS.length] }"></span><span class="legend-label">{{ d.name }}</span><span class="legend-value">{{ d.percentage }}%</span></div></div></div></div>
        </div>
      </template>
    </main>
  </div>
